...
import com.Game.Entities.Characters.Attacks.Attack;
...
public class CharacterSheet extends JPanel {
    ...
    public void assignAbilityScore(String score, int value) {
        int mod = (value - 10) / 2;
        abilityScores.put(score.toLowerCase(), new int[] { value, mod });
    ...
    public HashMap getMapForStat(String stat) {
        if (abilityScores.get(stat) != null) {
            return abilityScores;
        }
        else if (secondaryScores.get(stat) != null) {
            return abilityScores;
        }
        return null;
    }
    ...
    public void updateStat(JTextField source, HashMap map, String stat, int value) {
        ...
        if (source == null && components != null) {
            source = components.componentKeys.get(stat);
        }
        try {
            if (map != null) {
                if (map == abilityScores) {
                    assignAbilityScore(stat, value);
                    calculateStats();
                } else {
                    map.put(stat, value);
                }
                ...
                if (stat.equals("hp")) {
                    setHealth(value);
                    if (source != null) {
                        source.setText(Integer.toString(health()));
                    }
                }
            }
        }
